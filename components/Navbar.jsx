...
regex.test(part) ? (
<mark key={index} className="bg-yellow-300 text-gray-900 font-semibold">{part}</mark>
) : (
part
)
);
...
// Process categories similar to CategoryNavBar (moved before searchCategories)
const processedCategories = useMemo(() => {
let categoriesData = categoriesWithSubcategories;
...
// Search categories from Redux store
const searchCategories = useCallback((query, getCategorySlugFn) => {
if (!query || !processedCategories || processedCategories.length === 0) return [];
...
if (title.includes(lowerQuery) || englishName.includes(lowerQuery) || slug.includes(lowerQuery)) {
matches.push({
type: 'category',
id: category.id || category._id,
title: category.title || category.englishName || '',
slug: getCategorySlugFn ? getCategorySlugFn(category) : (category.slug || category.title || category.englishName || '')
});
if (matches.length >= 2) break; // Limit to 2 categories
}
}
...
// Get category slug function (defined later in component)
const getCategorySlugFn = (category) => {
if (category.slug && category.slug.trim() !== '') {
return category.slug;
}
const categoryTitle = category.title || category.englishName || '';
return categoryTitle ? categoryTitle.trim() : '';
};

// Search products and categories in parallel
const [productsResult, categoryMatches] = await Promise.all([
searchProducts(query).catch(() => ({ success: false, data: [] })),
Promise.resolve(searchCategories(query, getCategorySlugFn))
]);
...
const products = productsResult.data.slice(0, 3).map(product => ({
type: 'product',
id: product._id || product.id,
title: product.title || product.name || '',
category: product.categoryId?.title || product.category?.title || ''
}));
allSuggestions.push(...products);
}

// Add categories (limit to 2, but total should be max 5)
const remainingSlots = 5 - allSuggestions.length;
if (remainingSlots > 0) {
allSuggestions.push(...categoryMatches.slice(0, remainingSlots));
}
...
router.push(`/category/products?category=${encodeURIComponent(suggestion.slug)}`);
}
setShowSuggestions(false);
setSearchQuery("");
setSelectedSuggestionIndex(-1);
...
{/* Desktop Search Bar - Large and Centered */}
<div className="flex items-center flex-1 min-w-0 max-w-3xl mx-auto relative">
<form
onSubmit={handleSearch}
className="w-full flex items-stretch h-12 rounded-lg border border-gray-300 focus-within:border-[#7C2A47] focus-within:ring-2 focus-within:ring-[#7C2A47]/20 transition-all duration-200 bg-white overflow-hidden relative z-10"
>
<input
ref={searchInputRef}
type="text"
...
<div
ref={suggestionsRef}
className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-[100] max-h-80 overflow-y-auto"
>
<div className="px-3 py-2 border-b border-gray-200 bg-gray-50">
<h3 className="text-xs font-semibold text-gray-700 uppercase tracking-wide">SUGGESTIONS</h3>
</div>
{suggestionsLoading ? (
