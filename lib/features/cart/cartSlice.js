    ...
    if (typeof window !== 'undefined') {
        try {
            ...
            if (savedCart) {
                ...
                // If no valid items, ensure everything is cleared and remove from localStorage
                ...
                return {
                    ...
                    items: parsedCart.items || [] // Only include items if cartItems is valid
                }
            }
        } catch (error) {
...
const saveCartToStorage = (cart) => {
    if (typeof window !== 'undefined') {
        try {
            // Only save the essential cart data, not the entire state
            const cartData = {
...
// Double-check: if items array exists but cartItems is empty, clear items
if (initialCartData.items && Array.isArray(initialCartData.items) && initialCartData.items.length > 0) {
    if (!initialCartData.cartItems || Object.keys(initialCartData.cartItems).length === 0) {
        initialCartData.items = [];
    }
}
